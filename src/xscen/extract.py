...
from .indicators import registry_from_collection
...
def search_data_catalogs(  # noqa: C901
    ...
    *,
    ...
    if allow_conversion:
        ...
        cat_kwargs = {"registry": registry_from_collection(xc.IndicatorCollection.from_yaml(conversion_yaml))}
...
def _dispatch_historical_to_future(catalog: DataCatalog, id_columns: list[str] | None = None) -> DataCatalog:
    ...
    df = catalog.df.copy()
    df["same_hist_member"] = df[sim_id_no_exp].apply(lambda row: "_".join(row.fillna("nan").values.astype(str)), axis=1)
