...
class TestClimatologicalOp:
    ...
    def test_horizons_as_dim(self, xrfreq):
        ...
        assert out.actual_time.dims == ("horizon",) + ((next(iter(freq[xrfreq])),) if freq[xrfreq] else ())
        assert (out.horizon.values == ["2001-2010", "2006-2015", "2011-2020", "2016-2025", "2021-2030"]).all()
        assert (
            out.actual_time.values == np.array([ds.time.isel(time=slice(i, i + o)).values for i in range(0, ds.time.size - 5 * o, 5 * o)]).squeeze()
        ).all()
