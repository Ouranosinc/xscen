...
* If using the `rechunk` option, coordinates with dask chunks will now be loaded into memory before saving to avoid issues. (:pull:`673`).
* ``xs.io.estimate_chunks`` now always return the length of the dimension instead of -1 to avoid potential 'OverflowError: can't convert negative value to hsize_t'. (:pull:`673`).
