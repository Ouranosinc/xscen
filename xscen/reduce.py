...
def build_reduction_data(
    ...
    """
    Utility that constructs the input required for ensemble reduction. This will combine all variables into a single DataArray and
    stack all dimensions except "realization".

    Parameters
    ----------
    datasets: dict
        Dictionary of datasets in the format {"id": dataset}. This can be generated by calling .to_dataset_dict() on a catalog.
    xrfreqs: list
...
def _concat_criteria(criteria, ens):
    """
    Utility that combines all variables and dimensions except 'realization'
