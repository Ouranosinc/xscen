    ...
    for model in info_models:
        ...
        if yrs.size == 0:
            logger.info(
                ...
                f"({tas.time[-1].dt.year}) of the provided 'tas_src' file for {selected}."
            )
...
def _restrict_wl(df, restrictions: dict):
    ...
    tas_src = restrictions["tas_src"] or Path(__file__).parent / "data" / "IPCC_annual_global_tas.nc"

     # open nc
    annual_tas = xr.open_dataset(tas_src).tas

    fields = ('mip_era', 'source', 'experiment', 'member')
    if restrictions["ignore_member"] and "wl" not in restrictions:
        fields = fields[:-1]
    df["wl_name"] = df.apply(lambda r: '_'.join([r[field] for field in fields]))
    wl_source = ["_".join(parts) for parts in zip(*[annual_tas[field] for field in fields])]

    if "wl" in restrictions:
        to_keep = df["wl_name"].apply(get_warming_level, return_horizons=False, **restrictions).notnull()
    else:
        to_keep = df["wl_name"].isin(wl_source)
